import { INSTALLED_POWERS, MAX_POWERS } from './constants';
...
export const createPowerTable = () =>
	INTERVALS.map(interval => {
		...
		let powers = POWER_TYPES.map(powerType => {
			const installedPower = INSTALLED_POWERS[powerType];
			...
			const utilizationFactor =
				1 +
				POWER_COOF[powerType]
					.map((polystep, i) => polystep * Math.pow(interval, 6 - i))
					.reduce(sum);
